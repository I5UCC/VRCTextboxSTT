...
class SettingsWindow:
    def __init__(self, config, config_path):
        ...
        self.tkui = tk.Tk()
        self.tkui.minsize(475, 730)
        self.tkui.maxsize(475, 730)
        self.tkui.resizable(False, False)
        self.tkui.configure(bg="#333333")
        self.tkui.title("TextboxSTT - Settings")

        self.label_use_cpu = tk.Label(master=self.tkui, bg="#333333", fg="white", text='Use CPU', font=(self.FONT, 12))
        self.label_use_cpu.grid(row=0, column=0, padx=PADX_L, pady=PADY, sticky='es')
        self.value_use_cpu = tk.StringVar(self.tkui)
        self.value_use_cpu.set("yes" if bool(self.config["use_cpu"]) else "no")
        ...
        self.opt_use_cpu.grid(row=0, column=1, padx=PADX_R, pady=PADY, sticky='ws')
        self.label_use_cpu.bind("<Enter>", (lambda event: self.show_tooltip("Use CPU instead of GPU for transcription. This option is only available if you have a compatible GPU and CUDA installed, otherwise it will be disabled.")))
        self.label_use_cpu.bind("<Leave>", self.hide_tooltip)
        if not cuda_available:
            self.opt_use_cpu.configure(state="disabled")
        ...
        self.label_max_transcribe_time.grid(row=12, column=0, padx=PADX_L, pady=PADY, sticky='es')
        ...
        self.entry_max_transcribe_time.grid(row=12, column=1, padx=PADX_R, pady=PADY, sticky='ws')
